...
async def broadcast_updates(interval: int = 1):
    ...
    while True:
        try:
            ...
            await manager.broadcast({"type": "market", "data": market_data}, "market")

            # Get technical data
            technical_data = get_technical_data()
            await manager.broadcast(
                {"type": "technical", "data": technical_data}, "technical"
            )

            # Get performance data
            performance_data = get_performance_data()
            await manager.broadcast(
                {"type": "performance", "data": performance_data}, "performance"
            )
